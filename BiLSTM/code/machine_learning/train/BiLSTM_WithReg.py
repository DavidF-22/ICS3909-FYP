...
results_file_path = 'Saves/BiLSTM_training_WithReg_results.txt'
...
def BiLSTM(input_shape, reg_factor, dropout_rate, regularizer, learning_rate):    
    ...
    bilstm1 = layers.Bidirectional(layers.LSTM(units=128, 
                                               ...
                                               recurrent_regularizer=regularizer(reg_factor)))(pair_embeddings)
    ...
    bilstm2 = layers.Bidirectional(layers.LSTM(units=64, 
                                               return_sequences=True, 
                                               kernel_regularizer=regularizer, 
                                               recurrent_regularizer=regularizer(reg_factor)))(dropout1)
    ...
    dense = layers.Dense(units=512, activation='relu', kernel_regularizer=regularizer(reg_factor))(pooled)
...
def plot_training(history, dataset_name, regularizer_type, save_dir, count_models, count_plots):
    ...
    plt.savefig(os.path.join(save_dir, f'BiLSTM_{dataset_name}_MultiTest_{regularizer_type}_{count_models}.{count_plots}.png'))
    plt.close('all')
    ...
    for training_data_file, training_label_file in zip(training_data_files, training_labels_files):
        ...
        for regularizer_type in regularizers.keys():
            ...
            for reg_factor, dropout_rate in zip(reg_factors, dropout_rates): 
                ...
                # build model
                model = BiLSTM(input_shape, reg_factor, dropout_rate, regularizers[regularizer_type], learning_rate=args.learning_rate)
                ...
                model_path = os.path.join(save_dir, f"BiLSTM_multiTest_{regularizer_type}_{dataset_name}_{count_models}.keras")
