...
def main():
    ...
    for training_data_file, training_label_file in zip(training_data_files, training_labels_files):
        ...
        if config_results:
            ...
            # build model
            if args.ResNet_type.lower() == "small":
                final_model = build_resnet_small(input_shape, best_config['dropout_rate'], learning_rate)
            elif args.ResNet_type.lower() == "medium":
                final_model = build_resnet_medium(input_shape, best_config['dropout_rate'], learning_rate)
            elif args.ResNet_type.lower() == "large":
                final_model = build_resnet_large(input_shape, best_config['dropout_rate'], learning_rate)
            else:
                raise ValueError("!!! Invalid ResNet type. Only 'small', 'medium', or 'large' are recognised !!!")
        ...
        del encoded_data, encoded_labels, full_training_data, full_training_labels, final_model, config_results, best_config, best_f1
        cleanup()
